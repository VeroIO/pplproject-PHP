<?php 
    ...
    $offset = $page*$limit;
    $datas = json_decode(file_get_contents(API.'/api/admin/commentlist?offset='.$offset.'&limit='.$limit.'&access_token='.$_SESSION['jwt']));
                                ...
                                <div class="slimScrollDiv" style="position: relative; overflow: hidden; width: 100%; height: auto;"><div class="table-responsive" style="overflow: hidden; width: 100%; height: auto;"><table class="table table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        <tr>
                                            ...
                                            <td class="username"><?php echo $data->parrent_name; ?></td>
                                            <td><?php echo $data->cmt_for; ?></td>
                                            <td>
                                                <div class="toggle-custom toggle-inline">
                                                    <label class="toggle" data-on="ON" data-off="OFF">
                                                        <input type="checkbox" class="status" id="checkbox-toggle3" name="checkbox-toggle" <?php if($data->status == 1){ echo "checked";}?>>
  ...
  <div class="modal-dialog">
    ...
    <script>
        $(".status").change(function() {
            ...
            if(this.checked){
                $.post('<?php echo API ?>/api/user/able_cmt', {
                    id: id,
                ...
                $.post('<?php echo API ?>/api/user/disable_cmt', {
                    id: id,
                    access_token:access_token,
                }, function(data, status) {
                    $('#ketqua').modal('show');
